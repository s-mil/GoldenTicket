...
The Golden Ticket is designed to be a lightweight web based ticketing system written using the c/# launguage. The front end of of the ticketing system will be a responsive designed website so that it is accessible on as many devices as possible. The backend for the sight will be a microservices implementation hosted on a debian server featuirng sqllitem, the dot net core, identity server 4, letsencrupt, apched web server and more. The server will feature continuous deployment, when a revision is pushed to the master branch the server will on its check (every minute) downlowad and deploy the change and restart any changes that are needed. In this ticketing system there will be two users of the interface, technicians and admins/managers.
\begin(itemize}
	\item Technicians will have access to the ticket queue and assigned tickets.
	\item Admins will have access to the ticket queue, assigned tickets (if anay) and the admin portal.
	\item The ticket queue will consist of all tickets that are not marked as complete. The ticket at the top of the queue will be the tiket whose due date is closest to now / farthest past due. Their will be options to filter views on tickets, such as only tickets assigned to you, only tickets at x dificulty level, etc.
	\item A ticket will consist of the following items:
	\begin{itemize}
		\item  Title
		\item  Description
		\item Difficulty level
		\item a boolean isPriority that can be set by adminsitrators to bring the ticket to the top of the list.
		\item  Contact
		\item Company/department
		\item notes + time entrys
		\item assinded technition
		\item due date/ time
	\end{itemize}
	...
	\begin{itemize}
		...
		\item join date
	\end{itemize}
\item A Techniction will consist of the
\end{itemize}
